...
namespace Framework.Editor
{
    public class SQLite3Creator : EditorWindow
    {
        ...
        private string databasePathKey, databasePath;
        private string scriptSavePathKey, scriptSavePath;
        private string excelPathKey, excelPath;
        ...
        void OnEnable()
        {
            ...
            string[] splite = dataPath.Split('/');
            string key = splite[splite.Length - 2];

            databasePathKey = key + "DatabasePath";
            scriptSavePathKey = key + "ScriptSavePath";
            excelPathKey = key + "ExcelPath";
            databasePath = EditorPrefs.GetString(databasePathKey, "DB/static.db");
            scriptSavePath = EditorPrefs.GetString(scriptSavePathKey, "Assets/Scripts/Data/");
            excelPath = EditorPrefs.GetString(excelPathKey, dataPath + "/ExcelData/");
        ...
        void OnGUI()
        {
            ...
            if (isSingleExcel)
            {
                ...
                if (GUILayout.Button("Select", GUILayout.MaxWidth(45)))
                {
                    string path = EditorUtility.SaveFolderPanel("Script Save Path", dataPath + scriptSavePath, "");
                    if (!string.IsNullOrEmpty(path))
                    {
                        scriptSavePath = path.Replace(dataPath, "") + "/";
                        EditorPrefs.SetString(scriptSavePathKey, scriptSavePath);
                    }
                }
        ...
        [MenuItem("Assets/Framework/Create SQLite3 Table")]
        ...
        [MenuItem("Framework/Database/Create All SQLite3 Table")]
        ...
        [MenuItem("Framework/Database/Generate Static DB", priority = 0)]
        static void GenerateStaticDB()
        {
            window = CreateInstance<SQLite3Creator>();
            window.minSize = new Vector2(500, 600);
            window.maxSize = new Vector2(500, 2000);
            window.isSingleExcel = false;
            window.databasePath = "/StreamingAssets/DB/static.db";
            window.scriptSavePath = "/Scripts/SQLite3/Static/";
            window.excelPath = "/Design/StaticExcels/";
            window.Show();

            LoadAllExcel(Application.dataPath + window.excelPath);
        }

        [MenuItem("Framework/Database/Generate Dynamic DB", priority = 3)]
        static void GenerateDynamicDB()
        {
            window = CreateInstance<SQLite3Creator>();
            window.minSize = new Vector2(500, 600);
            window.maxSize = new Vector2(500, 2000);
            window.isSingleExcel = false;
            window.databasePath = "/StreamingAssets/DB/dynamic.db";
            window.scriptSavePath = "/Scripts/SQLite3/Dynamic/";
            window.excelPath = "/Design/DynamicExcels/";
            window.Show();
